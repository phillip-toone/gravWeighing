...
#Region "Start up Tasks"
   ...
   Public Sub SaveSpecialSettings()
      ...
      Data += Balance.BalanceSwitchEnabled & ","
...
#End Region
   ...
   Sub WeighFiltersFromCarosel(ByVal batch As Int32, ByVal endingLocation As Locations, ByVal startFilter As Int32)
      ...
      Dim EnvironmentalData() As String
      Static LastTemperature as double = 21.0
      Static LastHumidity As Double = 40.0
      ...
      Try
         ...
         Carosel.Home()
         Try
            EnvironmentalData = FileIO.ReadTextFile(BaseInfoPath & "\Humidity.txt").Split(Chr(44))   'Split on comma
            Experiment(batch).DryingTimes(Experiment(batch).WeighingStep).Temperature = CDbl(EnvironmentalData(0))
            LastTemperature = Experiment(batch).DryingTimes(Experiment(batch).WeighingStep).Temperature
            Experiment(batch).DryingTimes(Experiment(batch).WeighingStep).Humidity = CDbl(EnvironmentalData(1))
            LastHumidity = Experiment(batch).DryingTimes(Experiment(batch).WeighingStep).Humidity
         Catch ex As Exception
            'If you just happen to read this file when the humidity program is writing it, then an error can occur.
            Experiment(batch).DryingTimes(Experiment(batch).WeighingStep).Humidity = LastHumidity
            Experiment(batch).DryingTimes(Experiment(batch).WeighingStep).Temperature = LastTemperature
         End Try
            ...
            If Experiment(batch).Filter(Pass).Status = RunState.Active Then
               If Carosel.GetFilter(batch + 1, Pass) Then
                  ...
                  If ManageBarcodeData(batch, Pass, Lable) = True Then
                     ...
                     Balance.Zero()   'They want to tare before every weighing
   ...
   Sub ProcessExperimentData(ByVal batch As Int32)
      'This sub closes and processes an experiment.  It writes data to the appropriate files on the C drive
      Dim Data As String = ""
      Dim Pass As Int32
      ...
      Try
         If Experiment(batch).ExperimentType = ExpType.TareExperiment Then
            ...
            'Was, changed on 9/24/2021   Data += "Step 1 Weighing Environment -     Humidity: " & Format(Experiment(batch).DryingTimes(Experiment(batch).WeighingStep).Humidity, "0.0") & "      Temperature: " & Format(Experiment(batch).DryingTimes(Experiment(batch).WeighingStep).Temperature, "0.0") & vbCrLf
            Data += "Step 1 Weighing Environment -     Humidity: " & Format(Experiment(batch).DryingTimes(0).Humidity, "0.0") & "      Temperature: " & Format(Experiment(batch).DryingTimes(0).Temperature, "0.0") & vbCrLf
            'Was, changed on 9/24/2021   Data += "Step 2 Weighing Environment -     Humidity: " & Format(Experiment(batch).DryingTimes(Experiment(batch).WeighingStep).Humidity, "0.0") & "      Temperature: " & Format(Experiment(batch).DryingTimes(Experiment(batch).WeighingStep).Temperature, "0.0") & vbCrLf
            Data += "Step 2 Weighing Environment -     Humidity: " & Format(Experiment(batch).DryingTimes(1).Humidity, "0.0") & "      Temperature: " & Format(Experiment(batch).DryingTimes(1).Temperature, "0.0") & vbCrLf
            If Experiment(batch).WeighingStep > 2 Then
               'Was, changed on 9/24/2021   Data += "Step 3 Weighing Environment -     Humidity: " & Format(Experiment(batch).DryingTimes(Experiment(batch).WeighingStep).Humidity, "0.0") & "      Temperature: " & Format(Experiment(batch).DryingTimes(Experiment(batch).WeighingStep).Temperature, "0.0") & vbCrLf
               Data += "Step 3 Weighing Environment -     Humidity: " & Format(Experiment(batch).DryingTimes(2).Humidity, "0.0") & "      Temperature: " & Format(Experiment(batch).DryingTimes(2).Temperature, "0.0") & vbCrLf
            End If
            If Experiment(batch).WeighingStep > 3 Then
               'Was, changed on 9/24/2021   Data += "Step 4 Weighing Environment -     Humidity: " & Format(Experiment(batch).DryingTimes(Experiment(batch).WeighingStep).Humidity, "0.0") & "      Temperature: " & Format(Experiment(batch).DryingTimes(Experiment(batch).WeighingStep).Temperature, "0.0") & vbCrLf
               Data += "Step 4 Weighing Environment -     Humidity: " & Format(Experiment(batch).DryingTimes(3).Humidity, "0.0") & "      Temperature: " & Format(Experiment(batch).DryingTimes(3).Temperature, "0.0") & vbCrLf
            End If
